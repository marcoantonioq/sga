<?php
App::uses('MedicamentosBovinosController', 'Controller');

/**
 * MedicamentosBovinosController Test Case
 *
 */
class MedicamentosBovinosControllerTest extends ControllerTestCase {

/**
 * Fixtures
 *
 * @var array
 */
	public $fixtures = array(
		'app.medicamentos_bovino',
		'app.medicamento',
		'app.fazenda',
		'app.account',
		'app.calendario',
		'app.categoriacalendario',
		'app.group',
		'app.user',
		'app.sexo',
		'app.pessoa',
		'app.fazendas_pessoa',
		'app.cargo',
		'app.pessoas_cargo',
		'app.atalho',
		'app.evento',
		'app.categoriasevento',
		'app.ic',
		'app.bovino',
		'app.pasto',
		'app.retiro',
		'app.fornecimento',
		'app.centralcusto',
		'app.custo',
		'app.pagamento',
		'app.compra',
		'app.fornecedore',
		'app.fazendas_fornecedore',
		'app.patrimonio',
		'app.fabricante',
		'app.produto',
		'app.venda',
		'app.cliente',
		'app.clientes_fazenda',
		'app.centralcustos_venda',
		'app.categoriap',
		'app.centralcustos_compra',
		'app.movbovino',
		'app.pesagen',
		'app.categoriab',
		'app.sexobovino',
		'app.pelagen',
		'app.garrote',
		'app.leitepesagen',
		'app.ordenha',
		'app.raca',
		'app.bovinos_raca',
		'app.bovinos_evento',
		'app.semem',
		'app.doenca',
		'app.eventos_doenca',
		'app.alert',
		'app.alerts_group',
		'app.alerts_user'
	);

/**
 * testIndex method
 *
 * @return void
 */
	public function testIndex() {
	}

/**
 * testView method
 *
 * @return void
 */
	public function testView() {
	}

/**
 * testAdd method
 *
 * @return void
 */
	public function testAdd() {
	}

/**
 * testEdit method
 *
 * @return void
 */
	public function testEdit() {
	}

/**
 * testDelete method
 *
 * @return void
 */
	public function testDelete() {
	}

}
